...
sub name {
	my $either = shift;
	ref $either ? $either->{Name} : "an unamed $either";
}
...
sub named {
	my $class = shift;
	my $name = shift;
	my $self = { Name => $name, Color => $class->defualt_color };
	bless $self, $class;
}
...
sub color {
	my $either = shift;
	ref $either ? $either->{Color} : $either->defualt_color;
}

=head2 sound() 
=cut
sub sound {
	print "class must define sound";
}

=head2 set_name()
=cut
sub set_name {
	ref(my $self = shift) or print "Errr";
	$self->{Name} = shift;
}
